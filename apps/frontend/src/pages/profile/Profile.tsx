...
const KYC_BADGE: Record<string, { label: string; className: string }> = {
  verified: { label: 'KYC VERIFIED', className: 'bg-[rgba(156,192,165,0.22)] text-[#B6E0BE]' },
  pending: { label: 'VERIFYING…', className: 'bg-[rgba(242,206,124,0.22)] text-gold-light' },
...
export default function Profile() {
  ...
  return (
    <div>
      <div className="teal-card-gradient rounded-b-[26px] pb-5">
        ...
        <div className="flex items-center gap-3.5 px-6">
          <div className="flex h-[66px] w-[66px] shrink-0 items-end justify-center overflow-hidden rounded-[22px] bg-gradient-to-br from-gold-panelFrom to-[#E1AC46] shadow-[0_8px_18px_-8px_rgba(0,0,0,0.5)]">
            <img src="/illustrations/bee-peek.png" alt="" className="h-[58px] w-[58px] object-contain object-bottom" />
          </div>
          <div className="min-w-0 flex-1">
            <div className="truncate text-xl font-black leading-tight text-white">{user?.name}</div>
            <div className="truncate text-[13px] font-semibold text-teal-soft3">{user?.email}</div>
            <span className={`mt-1.5 inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-extrabold ${badge.className}`}>
              ✓ {badge.label}
            </span>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-3.5 px-5 pt-4 pb-6">
        <div>
          <div className="mb-2 pl-1 text-[11px] font-extrabold uppercase tracking-wide text-ink-faint">Connected Channels</div>
          <button
          ...
          >
            ...
            <div className="flex-1">
              <div className="text-[15px] font-extrabold text-ink">Telegram</div>
              <div className={`text-xs font-bold ${telegramConnected ? 'text-active-text' : 'text-ink-muted'}`}>
                {telegramConnected ? 'Alerts & receipts active' : 'Not connected'}
              </div>
            </div>
            <span
        ...
        <div>
          <div className="mb-2 pl-1 text-[11px] font-extrabold uppercase tracking-wide text-ink-faint">Settings</div>
          <div className="overflow-hidden rounded-[18px] bg-white shadow-[0_3px_12px_rgba(20,40,45,0.05)]">
            {SETTINGS_ROWS.map((row, i) => (
              ...
              >
                <span className="flex-1 text-[14.5px] font-extrabold text-ink">{row.label}</span>
        ...
        >
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#B24A3C" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round">
