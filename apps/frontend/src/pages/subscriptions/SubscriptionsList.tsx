        ...
        {loading ? (
          ...
          <div className="flex-1 bg-white rounded-[26px] shadow-[0_4px_16px_rgba(20,40,45,0.05)] flex flex-col items-center justify-center text-center px-[30px] py-[36px]">
            <img
              ...
              alt=""
              className="w-[160px] h-[160px] object-contain drop-shadow-[0_10px_16px_rgba(20,40,45,0.12)]"
            />
            <div className="text-[19px] font-black text-ink mt-2">
              No subscriptions yet!
            </div>
            <div className="text-[14px] font-semibold text-ink-muted leading-relaxed max-w-[250px] mt-1.5">
              Add Netflix, Spotify, DStv or any recurring bill and SubBee keeps
              it paid on time.
            </div>
            <button
          ...
          <>
            {subscriptions.map((sub) => (
              <SubscriptionRow
                key={sub.id}
                sub={sub}
                insufficient={isInsufficientFunds(sub, walletKobo)}
              />
            ))}
